...
export class PlatformSprite {
  ...
  private readonly texture_scale = Camera.px_per_unit / 160;
  private make_sprites() {
    if (this.data.len > 1) {
      if (this.data.len > 2) {
        const center_sprite = Sprite.from(ImageAssetHandler.get("tile_center"));
        center_sprite.scale.set((this.data.len - 2) * this.texture_scale, this.texture_scale);
      ...
      left_side_sprite.scale.set(this.texture_scale);
      ...
      right_side_sprite.scale.set(this.texture_scale);
      ...
      platform_sprite.scale.set(this.texture_scale);
